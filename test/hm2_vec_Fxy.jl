...
plot!(plt_Fn, xs, [φₙ(x, 1) - αₙ(x, 1) * F(x, 1) - k(1) * βₙ(x, 1) * G(x) * DH(1) for x in xs], label="F(x,1) BC")
plt_Fs = plot(title="F(x,0) and ∂F/∂y(x,0)", xlabel="x", ylabel="F and ∂F/∂y")
plot!(plt_Fs, xs, [φₛ(x, 0) - αₛ(x, 0) * F(x, 0) - k(0) * βₛ(x, 0) * G(x) * DH(0) for x in xs], label="F(x,0) BC")
plt_Fr = plot(title="F(1,y) and ∂F/∂x(1,y)", xlabel="y", ylabel="F and ∂F/∂x")
plot!(plt_Fr, ys, [φᵣ(1, y) - αᵣ(1, y) * F(1, y) - k(y) * βᵣ(1, y) * (DG(1) * H(y)) for y in ys], label="F(1,y) BC")
plt_Fl = plot(title="F(0,y) and ∂F/∂x(0,y)", xlabel="y", ylabel="F and ∂F/∂x")
plot!(plt_Fl, ys, [φₗ(0, y) - αₗ(0, y) * F(0, y) - k(y) * βₗ(0, y) * (DG(0) * H(y)) for y in ys], label="F(0,y) BC")
...
if status == :Symbolic
    @showprogress desc = "Computing C..." Threads.@threads for index in CartesianIndices((L,))
        ...
        # F(x,y) = C + G(x) * H(y) | ∇ ⋅ (k ∇F) = Dk(y) * ∂F/∂y + k(y) * (∂²F/∂x² + ∂²F/∂y²)
        # Dk(y) * ∂F/∂y = Dk(y) * G(x) * H'(y)
        aux6_ghx = quadgk(x -> G(x) * ψᵢ(i₀, x), 0, 1; rtol=1e-3, atol=1e-8)[1]
        aux6_ghy = quadgk(y -> Dk(y) * DH(y) * ψⱼ(j₀, y), 0, 1; rtol=1e-3, atol=1e-8)[1]
        aux6_gh = aux6_ghx * aux6_ghy
        aux6 = -1 * aux6_gh
        # k(y) * ∂²F/∂x² = k(y) * G''(x) * H(y)
        aux7_kx = quadgk(x -> D2G(x) * ψᵢ(i₀, x), 0, 1; rtol=1e-3, atol=1e-8)[1]
        aux7_ky = quadgk(y -> k(y) * H(y) * ψⱼ(j₀, y), 0, 1; rtol=1e-3, atol=1e-8)[1]
        aux7_k = aux7_kx * aux7_ky
        aux7 = -1 * aux7_k
        # k(y) * ∂²F/∂y² = k(y) * G(x) * H''(y)
        aux8_ghx = quadgk(x -> G(x) * ψᵢ(i₀, x), 0, 1; rtol=1e-3, atol=1e-8)[1]
        aux8_ghy = quadgk(y -> k(y) * D2H(y) * ψⱼ(j₀, y), 0, 1; rtol=1e-3, atol=1e-8)[1]
        aux8_gh = aux8_ghx * aux8_ghy
        aux8 = -1 * aux8_gh
...
results_path = joinpath(@__DIR__, "..", "results", "hm2", "L_$L")
mkpath(results_path)
